...
def debug_k_predicted():
    ...
    # 创建数据库连接
    db = Database()
    if not db.connect_to_test_db():
        print("连接测试数据库失败")
        return
    if not db.connect_to_prod_db():
        print("连接生产数据库失败")
        return
    ...
    query = "SELECT * FROM k_management WHERE K_predicted IS NOT NULL AND K_predicted != 0 LIMIT 5"
    if db.execute_query(db.prod_cursor, query):
        results = db.fetch_all(db.prod_cursor)
        print(f"找到 {len(results)} 条K_predicted不为空且不为0的记录")
        for row in results:
            print(f"记录: {row}")
