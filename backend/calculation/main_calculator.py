...
class MainCalculator:
    ...
    def train_stage1(self):
        """执行阶段1训练：使用training_days天的数据进行初始拟合，支持逐个points训练"""
        ...
        print(f"发现的points列表: {all_points}")
        
        # 使用stage1进行初始拟合
        self.model_params_by_points = {}
        
        for points in all_points:
            print(f"\n训练 Points: {points} 的基础参数...")
            
            # 过滤当前points的数据
            points_data = [data for data in training_data if data.get('points') == points]
            
            if not points_data:
                print(f"  Points: {points} 训练数据为空，使用默认参数")
                self.model_params_by_points[points] = {'a': 1.0, 'p': 0.8, 'b': 0.0003}
                continue
